...
def multiplication_chain(num):
    ...
    while len(str(num)) > 1:
        mn = 1  # вводный множитель
        for spi in str(num):
            mn = mn * int(spi)  # перемножаем цифры числа
        num = mn  # результат перезаписываем в num, чтобы повторить итерацию, пока не останется 1 цифра
        count_multy += 1  # с каждой итерации счетчик увеличивается на 1

...
def josephus_task(num_people, kill_num):
    people = list(range(1, num_people + 1))  # генерируем список
    index = 0
    while len(people) > 1:  # чистим список, пока не останется 1 элемент
        index = (index + kill_num - 1) % len(people)  # определяем индекс выбывшего
        people.pop(index)  # удаляем выбывшего из списка по найденному индексу
    survivor = people[0]
    return survivor

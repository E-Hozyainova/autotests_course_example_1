...
def repeats(our_str):
    """ Смотрит, сколько раз каждый символ уже встречался, и добавляет количество к символам с помощью постфикса
        формата _n.
                :param: our_str - строка
                :return: new_str - строка: буква_число повторений """

    kol = dict.fromkeys(our_str, 0)  # создаем словарь, где ключ - буква, значение - 0
    new_str = str()
    for s in our_str:  # разбиваем слово
        kol[s] += 1  # количество повторений буквы записываем в знаение словаря
        wer = kol[s]  # записываем в переменную значение повтрения буквы
        for _ in s:
            new_str += s + '_' + str(wer)  # собираем строку в заданном формате

...
def scrabble(word):
    """ Считает количество очков, полученное за слово.
                        :param: word - составленное слово
                        :return: points - количество очков """

    # создаем словарь, где буквы с одним номиналом очков будут ключом, а значением будет количество очков за букву
    word_points_dict = {'авеёинорст': 1, 'дклмпу': 2, 'бгья': 3, 'йы': 4, 'жзхцч': 5, 'фшэю': 8, 'щ': 10, 'ъ': 15}
    points = []  # пустой список очков
    for ip in word:  # итерируемся по буквам слова
        for key, velue in word_points_dict.items():  # итерируемся по парам словаря
            if ip in key:
                points.append(velue)  # если буква есть в ключе, то добавляем значение этого ключа в список очков

    points = sum(points)  # суммируем очки
    return points

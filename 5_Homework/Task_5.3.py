...
def everything_for_your_cat(cats_data):
    """ Оптимизирует хранение данных.
                    :param: cats_data - список кортежей
                    :return: our_str - строка: на отдельной строке хозяин и все его кошечки """

    our_str = ''
    cats_data_dict = {}
    for cats in cats_data:  # разбиваем список кортежей
        cat = cats[0] + ', ' + str(cats[1])  # записываем в переменную имя кошки и ее возраст
        cats_data_dict.setdefault(cats[2:], []).append(cat)  # добавляем в словарь - ИФ хозяина: кошечка и возраст
    for key, vel in cats_data_dict.items():
        our_str1 = (' '.join(key) + ': ', '; '.join(vel) + '\n')  # устанавливаем формат
        res = ''.join(our_str1)  # записывем в переменную распакованный кортеж
        our_str += str(res)
    return our_str
